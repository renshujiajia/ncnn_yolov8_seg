 ...
 * @brief Yolo类的构造函数
 * 
 * 该构造函数为Yolo类的默认构造函数，不接受任何参数，也不返回任何值。
 * 主要用于创建Yolo类的实例。
 */
Yolo::Yolo()
{
 ...
 * ~Yolo 析构函数
 * 
 * 本函数为类 Yolo 的析构函数，负责在对象销毁时执行必要的清理工作。
 * 由于本析构函数并未实现任何具体逻辑，因此不接受任何参数
 */
Yolo::~Yolo()
{
    printf("Yolo destroyed\n");
}

// 加载模型
bool Yolo::loadmodel(char* modelpath, char* parampath, bool use_gpu = true){
    if (use_gpu)
    {
        printf("will use gpu ...\n");
    }
    ...
    yolonet.opt.use_vulkan_compute = use_gpu;

    // 加载模型: 必须先加载参数然后再加载模型
    yolonet.load_param(parampath);
    yolonet.load_model(modelpath);
...
// 执行推理
bool Yolo::inference(cv::Mat& img, std::vector<Object>& objects){
...
// 结果绘制
void Yolo::drawresult(cv::Mat& img, std::vector<Object>& objects){
...
//    static const char* class_names[] = {"blur", "phone", "reflectLight", "reflection"};
    ...
    for (size_t i = 0; i < objects.size(); i++)
    {
        ...
        cv::rectangle(img, obj.rect, cc, 2);
